...
class RabbitMQEventSubscriber {
  constructor({ url, queueName, connection }) {
    this.url = url;
    this.queueName = queueName;
    this.connection = connection ?? null;
    this.channel = null;
    this.handlers = new Map(); // routingKey -> handler
  }
  ...
  async start() {
    ...
    await this.channel.consume(this.queueName, async (msg) => {
      ...
      if (!handler) {
        console.warn("[subscriber] no handler for", routingKey);
      ...
      try {
        ...
        this.channel.nack(msg, false, false); // dead-letter (or drop) — avoid infinite loops
      }
    });
  }
}
